...
function seedPositions() {
  const db = getDb();
  db.prepare(
    `INSERT INTO positions (ticker, asset_class, quantity, avg_price_cents, current_price_cents, market_value_cents, invested_cents, last_imported_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    "BBAS3",
    "ACAO",
  ...
  try {
    ...
    const source = new FakeSource(
      ...
      },
      {
        ...
        BTLG11: fii_fund("BTLG11", 7), // below min_dividend_yield_pct=6: passes; ordered last
      },
    );
    ...
    assert.equal(result.warnings[0], "[disclaimer]" + result.warnings[0].slice(11));
  } finally {
    tearDown(env.dir);
  }
});
...
test("suggestBuys: default top_n=3", async () => {
  ...
  try {
    ...
    const source = new FakeSource(
      {
        FII: [{ ticker: "MXRF11" }, { ticker: "HGLG11" }],
      },
      {
        MXRF11: quote("MXRF11", 10, 500_000_000),
        HGLG11: quote("HGLG11", 150, 800_000_000),
